...
from inmobiliaria.serializers import MiniPropiedadSerializer, MiniInteresadoSerializer
...
class AdminUsuarioSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ["id", "username", "email", "rol", "is_active", "aprobado", "perfil"]

    def get_perfil(self, obj):
        email = (obj.email or "").strip().lower()

        # ---- PROPIETARIO: preferir por usuario ----
        p = (
            Propietario.objects.filter(usuario=obj).first()
            or Propietario.objects.filter(email__iexact=email).order_by("-id").first()
        )
        if p:
            dir_p = (
        ...
        # ---- CLIENTE / INTERESADO: preferir por usuario ----
        c = (
            Interesado.objects.filter(usuario=obj).first()
            or Interesado.objects.filter(email__iexact=email).order_by("-id").first()
        )
        if c:
            return {
                "tipo": "CLIENTE",
...
class ReservaNotaAdminSerializer(serializers.ModelSerializer):
    autor_email = serializers.SerializerMethodField()

    class Meta:
        model = ReservaNotaAdmin
        fields = ("id", "texto", "created_at", "autor_email")

    def get_autor_email(self, obj):
        return getattr(obj.autor, "email", None)
...
class AdminReservaSerializer(serializers.ModelSerializer):
    ...
    notas_admin = ReservaNotaAdminSerializer(many=True, read_only=True)
    ...
    class Meta:
        ...
        fields = [
            "id", "estado", "vencida",
            "fecha", "expires_at", "monto_reserva", "notas", "activa",
            "propiedad", "interesado", "notas_admin",
        ]

    def get_vencida(self, obj):
        return obj.estado == "expirada"

class AdminSolicitudNotaSerializer(serializers.ModelSerializer):
    autor_email = serializers.SerializerMethodField()
...
class AdminSolicitudSerializer(serializers.ModelSerializer):
    # cliente = interesado en modo compacto
    interesado = MiniInteresadoSerializer(read_only=True)
    notas_admin = AdminSolicitudNotaSerializer(many=True, read_only=True)
