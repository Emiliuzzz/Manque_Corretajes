...
def notificar_propiedad_creada(sender, instance: Propiedad, created, **kwargs):
    """
    Si un PROPIETARIO crea una propiedad (aprobada=False por flujo) => notificar a ADMIN 
    """
    if not created:
        return
...
def contrato_post_save(sender, instance: Contrato, created, **kwargs):
    if not created:
        return

    # 1) Cambiar estado final de la propiedad
    nuevo_estado = "vendida" if instance.tipo == "venta" else "arrendada"
    Propiedad.objects.filter(id=instance.propiedad_id).update(estado=nuevo_estado)

    # 2) Desactivar reservas activas vigentes
    now = timezone.now()
    Reserva.objects.filter(
        propiedad_id=instance.propiedad_id,
        activa=True,
        expires_at__gt=now,
    ).update(activa=False)
    ...
    _notificar(propietario_user, f"Contrato de {t} creado",
               f"Se creó un contrato de {t} para '{prop.titulo}' por ${instance.precio_pactado:.0f}.",
               tipo="SISTEMA")
    _notificar(cliente_user, f"Tu contrato de {t}",
...
def notificar_pago_creado(sender, instance: Pago, created, **kwargs):
    ...
    monto_txt = _fmt_clp(instance.monto)

    titulo = "Pago registrado"
    msg_c = f"Se registró un pago de ${monto_txt} para tu contrato de '{prop.titulo}'."
    msg_p = f"Se registró un pago de ${monto_txt} en el contrato de '{prop.titulo}'."
